...
HELPER="$EXUI_ROOT/vendor/exui/build/tools/extract_utils.sh"
if [ ! -f "$HELPER" ]; then
...
setup_vendor "${DEVICE}" "${VENDOR}" "${EXUI_ROOT}" false "${CLEAN_VENDOR}"

extract "${MY_DIR}"/proprietary-files.txt "${SRC}" \
        "${KANG}" --section "${SECTION}"

DEVICE_BLOB_ROOT="${EXUI_ROOT}"/vendor/"${VENDOR}"/"${DEVICE}"/proprietary
